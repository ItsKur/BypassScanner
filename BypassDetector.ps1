<#
================================================================================
 BypassDetector.ps1 - Screenshare bypass / anti-forensics detector
================================================================================
 ...
 OFF by default and only runs if you pass -IncludeSysMainCheck. The optional
 -MonitorJava mode is read-only too: it only reads the process list, and its
 alerts (a notification, a sound, a window activation) are local UI that change
 no system state.

 USAGE
 -----
   .\BypassDetector.ps1
   .\BypassDetector.ps1 -OutputPath .\report.txt
   .\BypassDetector.ps1 -IncludeSysMainCheck -Quiet
   .\BypassDetector.ps1 -MonitorJava

 JAVA MONITOR (-MonitorJava)
 ---------------------------
 After the scan, optionally keep watch on the Minecraft client for the whole
 screenshare. It polls the process list and, the moment java closes, raises a
 Windows notification, plays the error sound and pulls this window to the
 foreground so a moderator cannot miss it. The loop runs until you press Q or
 Esc to end the screenshare. This is local UI only: still no network calls and
...
param(
    ...
    # compiles inline C#, so it is opt-in and stays out of the default execution
    # path. See the big comment block at Test-SysMainSuspendedThreads.
    ...
    # After the scan, continuously watch the Minecraft client (java.exe /
    # javaw.exe) for the duration of a screenshare. If java closes, raise a
    # Windows notification, play the error sound and bring this window to the
    # foreground. The loop runs until you press Q or Esc. See Watch-JavaProcess.
    [switch]$MonitorJava,

    # Seconds between java-presence polls when -MonitorJava is used.
...
# Set once the java monitor has run so the final keypress wait is skipped - the
# monitor already consumes the key that ends the screenshare.
$script:JavaMonitorRan = $false
...
function Test-SysMainSuspendedThreads {
    ...
    if (-not ('BypassDetector.ThreadProbe' -as [type])) {
        $cSharp = @'
using System;
using System.Runtime.InteropServices;

namespace BypassDetector {
...
'@
    ...
    $threads = @([BypassDetector.ThreadProbe]::GetThreads([uint32]$serviceProcessId))
...
function Invoke-AllChecks {
    ...
    foreach ($check in $checks) {
        $index++
        Write-Progress -Activity 'BypassDetector' -Status ("[{0}/{1}] {2}" -f $index, $checks.Count, $check.Name) `
            -PercentComplete (($index / $checks.Count) * 100)
        $results.Add((Invoke-Check -Id $check.Id -Name $check.Name -Body $check.Body))
    }
    Write-Progress -Activity 'BypassDetector' -Completed
    return $results.ToArray()
}
...
function Write-Report {
    ...
    Write-Line ''
    Write-Line '================================================================================' 'Cyan'
    Write-Line ' BypassDetector - screenshare bypass / anti-forensics scan' 'Cyan'
    Write-Line '================================================================================' 'Cyan'
...
#===============================================================================
# SECTION D2 - java presence monitor  [OPT-IN, -MonitorJava]
#
# READS  : the running process list, via Get-Process.
# WRITES : nothing to disk and nothing over the network. When java closes it
#          raises a desktop notification, plays the system error sound and pulls
#          this window forward - all local UI, changing no system state.
#
# During a screenshare the moderator needs the Minecraft client to stay open so
# they can inspect it. If the suspect closes java to dodge inspection or to
# destroy live evidence, this loop alerts the moment it happens and keeps
# watching until java returns or the moderator ends the screenshare.
#===============================================================================

# True when at least one Java (Minecraft client) process is running. Get-Process
# matches on the base name, so 'java' also covers java.exe and 'javaw' javaw.exe.
function Test-JavaRunning {
    $procs = @(Get-Process -Name 'java', 'javaw' -ErrorAction SilentlyContinue)
    return ($procs.Count -gt 0)
}

# Plays the Windows error sound. Falls back to a console beep on hosts without
# the SystemSounds API. Changes no state.
function Invoke-ErrorSound {
    try { [System.Media.SystemSounds]::Hand.Play(); return } catch { }
    try { [Console]::Beep(880, 600) } catch { }
}

# Brings this PowerShell window to the foreground. Uses the WScript.Shell COM
# object's AppActivate so no C# is compiled. Best-effort: some hosts refuse to
# steal focus, and that failure is swallowed.
...
# Raises a Windows notification - a tray balloon, which surfaces as a toast in
# the Windows 10/11 notification centre. Loads the shipped WinForms/Drawing
# assemblies; it compiles no C# of its own. Returns quietly if the host cannot
# show one.
function Show-JavaToast {
    ...
    try {
        ...
        # Give the shell a moment to render the balloon before the icon is
        # disposed, otherwise it can vanish before it is drawn.
...
# Fires all three alerts together when java has closed. Each is independent, so
# one failing (no sound device, no notification area) still leaves the others.
function Invoke-JavaClosedAlert {
    param([string]$Message)
    Invoke-ErrorSound
    Show-JavaToast -Title 'BypassDetector - java closed' -Message $Message
    Set-WindowForeground
}

# The monitoring loop. Polls every $IntervalSeconds and runs until the moderator
# presses Q or Esc. It waits for java to appear before it starts guarding, so
# launching the monitor a beat early raises no false alarm, then alerts on every
# open -> closed transition for as long as the screenshare lasts.
function Watch-JavaProcess {
    param([int]$IntervalSeconds = 3)

    # The loop has no exit unless a key can be read, so a redirected or headless
    # host must not enter it - that would hang forever with nothing to inspect.
    $canReadKeys = $true
    try { if ([Console]::IsInputRedirected) { $canReadKeys = $false } } catch { }
    if ($canReadKeys) { try { $null = [Console]::KeyAvailable } catch { $canReadKeys = $false } }
    if (-not $canReadKeys) {
        Write-Host ''
        Write-Host ' Java monitor needs an interactive console (a key ends it); skipping.' -ForegroundColor Yellow
        return
    }

    Write-Host ''
    Write-Host '--------------------------------------------------------------------------------' -ForegroundColor Cyan
    Write-Host ' JAVA MONITOR - watching the Minecraft client for the duration of the screenshare' -ForegroundColor Cyan
    Write-Host '--------------------------------------------------------------------------------' -ForegroundColor Cyan
    Write-Host (" Polling every {0}s. Press Q or Esc to end the screenshare and stop monitoring." -f $IntervalSeconds) -ForegroundColor DarkGray

    $seenJava = Test-JavaRunning
    if ($seenJava) {
        Write-Host ' java is running. Watching.' -ForegroundColor Green
    }
    else {
        Write-Host ' Waiting for java (java.exe / javaw.exe) to start...' -ForegroundColor Yellow
    }
    $wasRunning = $seenJava

    while ($true) {
        # End the screenshare on Q or Esc. Drain the whole buffer so a burst of
        # keystrokes does not leave one queued for the next read.
        try {
            while ([Console]::KeyAvailable) {
                $key = [Console]::ReadKey($true)
                if ($key.Key -eq 'Q' -or $key.Key -eq 'Escape') {
                    Write-Host ''
                    Write-Host ' Screenshare concluded. Java monitor stopped.' -ForegroundColor Cyan
                    return
                }
            }
        }
        catch {
            Write-Host ' Console input became unreadable; stopping the java monitor.' -ForegroundColor Yellow
            return
        }

        $running = Test-JavaRunning

        if (-not $seenJava -and $running) {
            $seenJava = $true
            Write-Host (" [{0}] java started - now watching." -f (Get-Date -Format 'HH:mm:ss')) -ForegroundColor Green
        }
        ...
        if ($seenJava) {
            if ($wasRunning -and -not $running) {
                ...
                Write-Host ''
                Write-Host (" [{0}] ALERT: java has CLOSED during the screenshare." -f $stamp) -ForegroundColor Red
                Invoke-JavaClosedAlert -Message ("java closed at {0}. The Minecraft client is no longer running." -f $stamp)
            }
            elseif (-not $wasRunning -and $running) {
                Write-Host (" [{0}] java is running again." -f (Get-Date -Format 'HH:mm:ss')) -ForegroundColor Green
            }
        }

        $wasRunning = $running

        # Sleep the poll interval in short slices so Q/Esc is picked up promptly
        # rather than only at the top of the next poll.
        $elapsedMs = 0
        $intervalMs = $IntervalSeconds * 1000
        while ($elapsedMs -lt $intervalMs) {
...
function Wait-ForKeyPress {
    # The java monitor already waited for, and consumed, the key that ends the
    # run, so there is nothing left to hold the window open for.
    if ($script:JavaMonitorRan) { return }
...
try {
    if (-not (Test-Elevated)) {
        Write-Host ''
        Write-Host ' BypassDetector must be run as Administrator.' -ForegroundColor Red
        Write-Host ''
    ...
    # Optional live phase: keep watch on the Minecraft client until the moderator
    # ends the screenshare. This runs after the report so the snapshot is on
    # screen first, and it consumes its own end key, so Wait-ForKeyPress is
    # skipped afterwards (see $script:JavaMonitorRan).
    if ($MonitorJava) {
        $script:JavaMonitorRan = $true
        Watch-JavaProcess -IntervalSeconds $MonitorIntervalSeconds
    }
    ...
    Write-Host ''
    Write-Host ' BypassDetector could not complete the run.' -ForegroundColor Red
